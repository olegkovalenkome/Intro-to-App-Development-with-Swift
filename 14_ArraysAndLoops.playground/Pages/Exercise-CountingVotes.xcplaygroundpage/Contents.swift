...
if yesVotes > noVotes {
    ...
    print("We shouldn't do it")
} else {
    print("Let's vote again")
}
...
func voteCheck(votes: [Bool]) -> String {
    var yesVotes = 0
    var noVotes = 0
    ...
    if yesVotes > noVotes {
        return "We should do it!  \(yesVotes) - yes, \(noVotes) - no"
    } else if yesVotes < noVotes {
        return "We shouldn't do it!  \(yesVotes) - yes, \(noVotes) - no"
    } else {
        return "Let's vote again!  \(yesVotes) - yes, \(noVotes) - no"
    }
}

voteCheck(votes: shouldMascotChangeVotes)
voteCheck(votes: shouldInstallCoffeeVendingMachineVotes)
voteCheck(votes: shouldHaveMorePollOptionsVotes)
